...
# shows misclassified images using GradCAM
def show_misclassified_img_gradcam(model, misclassified_images, misclassified_labels, misclassified_predictions, classes):
    print("Misclassified Images using GradCAM:")
    # Apply GradCAM visualization to the first 10 misclassified images
    for i in range(10):
        image = misclassified_images[i].unsqueeze(0)
        ...
        # Get the heatmap for the prediction class
        heatmap = gradcam(image, target_class=prediction)

        # Visualize the heatmap on the original image
        image_with_heatmap = visualize_cam(heatmap, image)

        # Display the original image, misclassified label, and predicted label
        plt.imshow(image[0].cpu().permute(1, 2, 0))  # Display the original image
        plt.title(f"Label: {classes[prediction]}\nPrediction: {classes[label]}")
        plt.show()

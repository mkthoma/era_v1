...
def show_class_samples(train_loader, classes):
    ...
    for idx, class_name in enumerate(classes):
        if class_name in images_per_class:  # Check if class exists in the dictionary
            ...
            for i in range(5):
                ...
                img = np.transpose(images[i], (1, 2, 0)) / 2 + 0.5
                ax.imshow(img)
                if i == 0:
...
def show_random_samples(train_loader,classes):
    # obtain one batch of training images
    dataiter = iter(train_loader)
    images, labels = next(dataiter)
    images = images.numpy() # convert images to numpy for display
...
def show_image_rgb(train_loader, classes):
    # Obtain one batch of training images
    dataiter = iter(train_loader)
    images, labels = next(dataiter)
    images = images.numpy() # Convert images to numpy for display
